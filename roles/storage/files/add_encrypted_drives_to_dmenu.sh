...
# This will be dynamically generated based on the new system's UUIDs
declare -A partitions=(
    ["/dev/disk/by-uuid/{{ backup_partition_uuid }}"]="/backup"
    ["/dev/disk/by-uuid/{{ secrets_partition_uuid }}"]="/secrets" 
    ["/dev/disk/by-uuid/{{ toast_partition_uuid }}"]="/toast"
)
...
CHOSEN=$(echo -e "$OPTIONS" | dmenu -i -fn 'Source Code Pro-14' -nb '#000000' -nf '#C5C8C6' -sb '#F0C674' -sf '#000000' -p "[mount]" -l 10)
...
if [ -n "$TARGET_PART" ]; then
    ...
    if [ "$STATUS" == "unmounted" ]; then
        ...
        # Send notification
        notify-send "Storage" "$MOUNT_POINT mounted successfully"
    else
        ...
        # Send notification
        notify-send "Storage" "$MOUNT_POINT unmounted successfully"
    fi
else
